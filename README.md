...
Этот Python-скрипт предназначен для парсинга информации о NFT-подарках Telegram (`t.me/nft/COLLECTION_SLUG-ID`). Он извлекает метаданные о каждом NFT, а также может скачивать связанные с ним .TGS файлы (анимированные стикеры) и .PNG файлы (паттерны фона).
